...
repl f parse initial = do
    ...
    maybe failAction succeedAction (parse line) where
        ...
        -- If nothing immediately goes wrong, we take the a and attempt to apply it to receive a
        -- new state.
        succeedAction a = do
...
commandParse (args, fs, [])
    ...
    where vstring = name ++ " version " ++ showVersion version
          name    = "JustSudoku"
...
main = do
    ...
    do
        ...
        -- Game header.
        putStrLn $ "This is JustSudoku, version " ++ showVersion version ++ " \n"
